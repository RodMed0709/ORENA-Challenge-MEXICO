...
Folder-private; run it directly.
    ...
    python submissions/04-rung40-conn4e5-ep23/test_normalize_answer.py
...
# Submission 03 sliced up to `@torch.no_grad()`, the decorator on its `answer_one`.
# This container batches through vLLM and has no such decorator, so the sentinel is
# the next top-level def instead. Asserted, because a silent slice miss would exec
# nothing and the test would "pass" having checked no code at all.
_end = _SRC.index("def answer_batch(", _start)
assert "def clamp_class_tokens" in _SRC[_start:_end], "source slice missed the functions under test"

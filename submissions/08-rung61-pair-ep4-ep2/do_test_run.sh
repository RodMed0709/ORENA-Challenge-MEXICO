...
# Pass ALLOW_CPU through when the caller sets it. Inert when unset, so the harness stays
# byte-identical to submission 03's for a normal GPU run. Without this there is no way to
# exercise the container's wiring on a box with no GPU -- and the wiring is exactly what
# was broken in submission 04, twice, undetected until it had spent a slot.
EXTRA_ENV=()
if [ -n "${ALLOW_CPU:-}" ]; then
    EXTRA_ENV=(--env "ALLOW_CPU=${ALLOW_CPU}")
    echo "=+= ALLOW_CPU=${ALLOW_CPU} -- wiring smoke, generation will be very slow"
fi

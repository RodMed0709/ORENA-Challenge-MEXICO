...
- [[class-imbalance-not-counting]] — 🔴 the failure is **per-CLASS, not per-count**: `gallstone` recall **0.000** (14 train ex.), `needle` 0.511, and a **phantom class** (`silicone loop`, 435 train ex., 0 in val) that is a guaranteed FP. **The ViT LoRA DID work** — `needle` +17.8 pts — `bucket_mean` hid it. But **68% of misses are on classes with >400 examples**, so rebalancing has a low ceiling.

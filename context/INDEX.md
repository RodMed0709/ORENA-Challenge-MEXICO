...
- [[count-calibration-dead]] — 🔴 **post-hoc count calibration is DEAD**, measured three ways (05c): oracle LUT gain **+0.026** vs the **+0.155** needed, ID↔OOD transfer **−0.016** (negative), and `argmax_injective` **False**. Mechanism: on the dominant template true values **2, 3 and 4 all share the same modal prediction (1)** (pooled over all 2,094 rows the collision shifts one step: 3 and 4 share 2) — a LUT can only send a prediction one place, so it trades one error for another. **Quote the scope.** **Not fixable with more data.** Leaves synthetic-counting SFT as the only lever left in the group that owns the gap.
...
- [[inference-only-input-tests-biased]] — 🔴 **an inference-only test of an INPUT-side lever is biased NEGATIVE** (−0.056 at inference vs +0.021 trained-with); the honest test is to TRAIN with it. Re-scopes rung 11 + rung 12 branch A/B. Does NOT touch the output family. Corroborated by Jong 2025 (*Endoscopy*) + Medeiros 2026.
- [[pooled-screening-manufactures-winners]] — pooled ranking of image transforms is confounded by between-video appearance (`tophat` +0.0043 → **−0.0172** within video); `within_video()` is the standard gate, and `wv_delta` must be read with its mean, not only its max.
- [[max-pixels-not-a-lever]] — 🔴 **no frame exceeds the pixel cap** (px_max 921,600 = the cap), so raising `max_pixels` adds nothing. Closes the lever [[latency-budget-is-pooled]] had re-opened.
- [[number-is-an-annotation-ceiling]] — ⚠️ **UPPER BOUND**: the counting label moves ±0.86 between frames ≤1 s apart (56.6 % of pairs) against a model MAE of 1.01; blind human r=−0.17 vs model +0.43. Bounds the recoverable gap — it does **not** prove the labels are wrong.
- [[headline-arithmetic-four-cells]] — `bucket_mean` is 4 equal cells and `fo_class` is 71 %/83 % of the `object_recognition` pair, so a `fo_class` +0.02 buys **+0.0035** of headline in ID only, **+0.0077** in both distributions. Ask "does it move a whole cell?", not "does it help?".

...
## Pending / blocked
- **Next experiment for rung 06: UNDECIDED.** Two candidates were cleared out of the way today, both cheaply: the 7.5 h `vit_lr` re-run (rationale disconfirmed — `number` decays with the ViT frozen too) and the epoch-1 checkpoint switch (**T7 measured it: epoch 2 is better in both arms, we did not own a better checkpoint**). Phase-3 #2 (**rank probe r=8→32**) is next in THE_MAP's order. See [[checkpoint-selection-vs-number]] **including its retraction**.

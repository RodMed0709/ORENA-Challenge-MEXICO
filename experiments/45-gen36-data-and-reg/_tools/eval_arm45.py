...
class EvalConfig:
    ...
    # 🔴 PER-PERSON CHECKOUT, and the shared `~/storage/repo` is GONE (retired
    # 2026-08-17, deliberately WITHOUT a symlink: two people now work the box at the
    # same time, and a symlink would let the old path write silently into somebody
    # else's copy). Rodrigo's is `repo_rod`; set this to yours.
    #
    # Whichever it is, it must carry `vendor/orena-focus` — the SDK is pip-installed
    # in NO env on that machine, `import focus` raises ModuleNotFoundError even in
    # orena-quant — and rung 23's `_tools/screen_engine.py`. Both ship in the repo,
    # so a checkout synced with `git ls-files` has them.
    #
    # `~/storage/ESTADO.md` (regenerated by `~/storage/estado.py`) says which
    # checkout is at which commit and which one a running GPU process belongs to.
